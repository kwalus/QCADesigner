...
{
  return ((my_qcell->y > my_qcell->cell_dots[0].y - OTHRESHOLD) &&
	  (my_qcell->y < my_qcell->cell_dots[0].y + OTHRESHOLD));
...
void Cell::handleParcel(parcel * p)
{
  ...
  if ((clock_zone == SWITCH) && (!isInput())) {
	...
	sendParcelsToNeighbors(c_info->getOldState(), c_info->getNewState(),
			       c_info->getOrientation(), (Cell *) p->source());
	parcel::deleteParcel(p);
	return;
      }
      // Normal ripper case
      else if ((normalized_angle > RIPANGLE1 - RIPTHRESHOLD) &&
	       (normalized_angle < RIPANGLE1 + RIPTHRESHOLD)) {
	bias -= p->getCellInfo()->getOldState();
	bias += p->getCellInfo()->getNewState();
      }
      // Inverting ripper case
      else if ((normalized_angle > RIPANGLE2 - RIPTHRESHOLD) &&
	       (normalized_angle < RIPANGLE2 + RIPTHRESHOLD)) {
	bias += p->getCellInfo()->getOldState();
	bias -= p->getCellInfo()->getNewState();
      }
    }
    // Sender and receiver have same orientation
      ...
      if (getOrientation() == 0) {
	// normal wire
	if ((normalized_angle < NORMALTHRESHOLD) ||
	    (normalized_angle > PI_OVER_TWO - NORMALTHRESHOLD)) {
	  bias -= p->getCellInfo()->getOldState();
	  bias += p->getCellInfo()->getNewState();
	}
	// inverting wire
	else if ((normalized_angle < INVANGLE + INVTHRESHOLD) &&
		 (normalized_angle > INVANGLE - INVTHRESHOLD)) {
...
void Cell::sendParcelsToNeighbors(const int old_state, const int new_state,
				  const int dest_orientation, const Cell * src_cell)
{
  
  if (!(new_state == UND || new_state == LOW || new_state == HIGH ||
	old_state == UND || old_state == LOW || old_state == HIGH)){
    printf("BAD VALUE!  Old: %d, New: %d \n", old_state, new_state); 
    fflush(stdout);
    printCellInfo();
    exit(1);
  }
  ...
  CellNeighborSet::iterator end_iter = neighbors->end();
  for (CellNeighborSet::iterator iter = neighbors->begin(); iter != end_iter; iter++) {
    if ((dest_orientation == (*iter)->neighbor->getOrientation()) &&
	((*iter)->neighbor != src_cell)) {
      parcel *p = parcel::newParcel();
      cellInfo *c_info = new cellInfo;
