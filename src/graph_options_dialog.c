...
void get_graph_options_from_user (GtkWindow *parent, struct TRACEDATA *traces, int icTraces, struct TRACEDATA *clocks, int icClocks)
  ...
  if ((NULL == traces && NULL == clocks) ||
    ...
    message_box (parent, MB_OK, "What Graph Options ?", "There are no graph options available for selection !") ;
    return ;
    }
...
void create_graph_options_dialog (graph_options_D *dialog)
  ...
  gtk_signal_connect_object (GTK_OBJECT (dialog->dlgGraphOptions), "delete_event", (GtkSignalFunc)gtk_widget_hide,
      	      	      	     GTK_OBJECT (dialog->dlgGraphOptions)) ;
  gtk_signal_connect_object (GTK_OBJECT (dialog->btnCancel), "clicked", (GtkSignalFunc)gtk_widget_hide,
      	      	      	     GTK_OBJECT (dialog->dlgGraphOptions)) ;
  gtk_signal_connect (GTK_OBJECT (dialog->btnOK), "clicked", (GtkSignalFunc)graph_options_btnOK_clicked,
      	      	      	     dialog->dlgGraphOptions) ;
  }
