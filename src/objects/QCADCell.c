...
GdkColor clrCluster = {0, 42405,10794,10794} ;
		...
		for (Nix = 0 ; Nix < cell->number_of_dots ; Nix++)
		{
			...
			if (QCAD_CELL_MODE_CLUSTER == cell->cell_options.mode) {
				...
				if (QCAD_CELL_INPUT == cell->cell_function) {
					memcpy (&(QCAD_DESIGN_OBJECT (cell)->clr), &clrBlue, sizeof (GdkColor)) ;
				}
				else {
					if (QCAD_CELL_FIXED == cell->cell_function) {
						...
						if (QCAD_CELL_OUTPUT == cell->cell_function) {
							memcpy (&(QCAD_DESIGN_OBJECT (cell)->clr), &clrYellow, sizeof (GdkColor)) ;
						}
						else {
