...
static double get_potential (double x, double y, double z, int Nx, int Ny, int Nz, double dx, double dy, double dz, int xmin, int ymin)  ;
...
static EXTREME_POTENTIALS extreme_potential (QCADElectrode *electrode, double z)
  ...
  // This assumes a sin function
	  ret.max = search_min_max(pot_grid,1,Nx*Ny*Nz);
  ret.min = search_min_max(pot_grid,0,Nx*Ny*Nz);
...
static double get_potential (double x, double y, double z, int Nx, int Ny, int Nz, double dx, double dy, double dz, int xmin, int ymin) 
  {
	  ...
	  x1 = (((x-xmin)/dx - floor((x-xmin)/dx)) >= dx) ? ceil((x-xmin)/dx) : floor((x-xmin)/dx);
	  y1 = (((y-ymin)/dy - floor((y-ymin)/dy)) >= dy) ? ceil((y-ymin)/dy) : floor((y-ymin)/dy);
	  z1 = ((z/dz) - floor(z/dz) >= dz) ? ceil(z/dz) : floor(z/dz);
	  
	  loc = x1*Ny*(Nz-1) + y1*(Nz-1) + z1;
	  return pot_grid[(int)loc];
  }
				...
				for (k = 0; k < Nz; k++) {
					if (k == 0) {
						...
						if (!elec_flag) {
							if (i == 0) {
								if (j == 0) {
									Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j+1)*Nz + k])/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2] - 2*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)*er_array[k]/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - er_array[k]/dz2);
								}
								else {
									if (j == (Ny-1)) {
										Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j-1)*Nz + k])/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2] - 2*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)*er_array[k]/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - er_array[k]/dz2);
										...
										Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[i*Ny*Nz + (j+1)*Nz + k] + Grid[i*Ny*Nz + (j-1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2] - 2*Grid[i*Ny*Nz + j*Nz + k+1])*(er_array[k])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - er_array[k]/dz2);
									}
								}
							}
							else {
								if (i == (Nx-1)) {
									...
									if (j == 0) {
										Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2] - 2*Grid[i*Ny*Nz + j*Nz + k+1])*er_array[k]/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - er_array[k]/dz2);
									}
									else {
										if (j == (Ny-1)) {
											Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j-1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2] - 2*Grid[i*Ny*Nz + j*Nz + k+1])*er_array[k]/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - er_array[k]/dz2);
											...
											Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[i*Ny*Nz + (j-1)*Nz + k] + Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2] - 2*Grid[i*Ny*Nz + j*Nz + k+1])*(er_array[k])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - er_array[k]/dz2);
										}
									}
								}
							}	
						...
						if (k == (Nz-1)) {
							...
							if (k == (Nz-2)) {
								if (i == 0) {
									if (j == 0) {
										Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
									}
									else {
										if (j == (Ny-1)) {
											Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j-1)*Nz + k])*er_array[k]/dy2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
											...
											Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[i*Ny*Nz + (j-1)*Nz + k]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
										}
									}
								}
								else {
									if (i == (Nx-1)) {
										...
										if (j == 0) {
											Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
										}
										else {
											if (j == (Ny-1)) {
												Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j-1)*Nz + k])*er_array[k]/dy2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
												...
												Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[i*Ny*Nz + (j-1)*Nz + k] + Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
											}
										}
									}
								}
								...
								if (k == floor(cell_layer/dz)+1) {
									Grid[w] = Grid[w+1] - (er_array[k+1]/er_array[k])*(Grid[w+2]-Grid[w+1]);
								}
								
								else {
									if (i == 0) {
										if (j == 0) {
											Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2]*(er_array[k+1]) + (er_array[k+2]-2*er_array[k+1]-er_array[k])*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
										}
										else {
											if (j == (Ny-1)) {
												Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j-1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2]*(er_array[k+1]) + (er_array[k+2]-2*er_array[k+1]-er_array[k])*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
												...
												Grid[w] = ((Grid[w]+Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[i*Ny*Nz + (j-1)*Nz + k]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2]*(er_array[k+1]) + (er_array[k+2]-2*er_array[k+1]-er_array[k])*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
											}
										}
									}
									else {
										if (i == (Nx-1)) {
											...
											if (j == 0) {
												Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2]*(er_array[k+1]) + (er_array[k+2]-2*er_array[k+1]-er_array[k])*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
											}
											else {
												if (j == (Ny-1)) {
													Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[w]+Grid[i*Ny*Nz + (j-1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2]*(er_array[k+1]) + (er_array[k+2]-2*er_array[k+1]-er_array[k])*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
													...
													Grid[w] = ((Grid[(i-1)*Ny*Nz + j*Nz + k] + Grid[(i+1)*Ny*Nz + j*Nz + k])*er_array[k]/dx2 + (Grid[i*Ny*Nz + (j-1)*Nz + k] + Grid[i*Ny*Nz + (j+1)*Nz + k])*er_array[k]/dy2 + (Grid[i*Ny*Nz + j*Nz + k+2]*(er_array[k+1]) + (er_array[k+2]-2*er_array[k+1]-er_array[k])*Grid[i*Ny*Nz + j*Nz + k+1])/dz2)/(2*er_array[k]/dx2 + 2*er_array[k]/dy2 - (2*er_array[k]-er_array[k+1])/dz2);
												}
											}
										}
									}	
